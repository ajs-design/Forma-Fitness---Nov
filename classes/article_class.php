...
class articles{
	...
	public function a_of_month_link(){
		foreach ($this->conn->query("SELECT * FROM articles WHERE a_of_month = '1'") as $row){
			$month = date_to_month($row['a_date']);
			$year = date_to_year($row['a_date']);
			echo 'index.php?p=articles&filter=month&year='.$year.'&filter_desc='.$month.'&filter_result='.$row['a_title'].'&filter_result_id='.$row['a_id'];
		}	
	}
	
	public function a_of_month_id(){
		foreach ($this->conn->query("SELECT a_id FROM articles WHERE a_of_month = '1'") as $row){
			echo $row['a_id'];
		}	
	}
