...
class client{
	...
	public function display_programs($id){
			$return = '';
			date_default_timezone_set('Europe/London');
			$date = date('Y-m-d H:i:s');
			
			//First get all current programs
			foreach($this->conn->query("SELECT * FROM client_program WHERE c_id = '".$id."' AND cprog_datecompleted = '0000-00-00 00:00:00'") as $prog){
				$prog_id = $prog['cprog_id'];
			$return .= '<h3>Current Program</h3>';
			$return .= '<p>Date set: <i>'.date('D jS Y', strtotime($prog['cprog_dateset'])).'</i></p>';
			$return .= '</br><p><strong>Routines:</strong></p>';
			$return .= '<ol>';
				$arr = '';
				foreach($this->conn->query("SELECT ro_id FROM cprog_routine_link WHERE cprog_id = '".$prog_id."'") as $row){
					foreach($this->conn->query("SELECT ro_title FROM routine WHERE ro_id = '".$row['ro_id']."'") as $routine){
						$return .= '<li>'.$routine['ro_title'].'</li>';
					}
				}
			$return .= '</ol>';	
			}	
	...
	public function display_program_goals($c_id){
		$return = '<h3>Current Goals</h3><hr>';
			foreach($this->conn->query("SELECT * FROM client_goals WHERE c_id = '".$c_id."' ORDER BY cg_target_date ASC") as $goal){
				$return .= '<p><strong>Goal: </strong>'.$goal['cg_goal'].'</p></br>';
				$return .= '<p><strong>Target Date: </strong>'.date('D jS F Y', strtotime($goal['cg_target_date'])).'</p></br>';
				$return .= '<p><strong>Date Set: </strong>'.date('D jS F Y', strtotime($goal['cg_set_date'])).'</p><hr>';
			}
		$return .= '<hr><h3>Past Goals</h3><hr>';
			foreach($this->conn->query("SELECT * FROM client_completed_goals WHERE c_id = '".$c_id."' ORDER BY cg_target_date ASC") as $com_goal){
				$return .= '<p style="color:#eeeeee;"><strong>Goal: </strong>'.$com_goal['cg_goal'].'</p></br>';
				$return .= '<p style="color:#eeeeee;"><strong>Target Date: </strong>'.date('D jS F Y', strtotime($com_goal['cg_target_date'])).'</p></br>';
				$return .= '<p style="color:#eeeeee;"><strong>Date Set: </strong>'.date('D jS F Y', strtotime($com_goal['cg_set_date'])).'</p><hr>';
			}
		echo $return;
	}
	
	public function display_routine_charts($c_id, $training){
		$return = '';
		foreach($this->conn->query("SELECT cprog_id FROM client_program WHERE c_id = '".$c_id."' ORDER BY cprog_dateset DESC LIMIT 1") as $row){
			$prog_id = $row['cprog_id'];
		}
		foreach($this->conn->query("SELECT ro_id FROM cprog_routine_link WHERE cprog_id = '".$prog_id."'") as $routine){
				$return .= $training->display_routine_chart($routine['ro_id']);
	...
	public function display_stats($c_id){
	...
	foreach($this->conn->query("SELECT date_taken FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<th scope="col">'.date('jS M y',strtotime($row['date_taken'])).'</th>';
		}
	$return .= '
						</tr>
					</thead>
	...
	foreach($this->conn->query("SELECT c_weight FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_weight'].'</td>';
		}
			$return .= '</tr>
						<tr>
							<th scope="row">Height</th>';
	foreach($this->conn->query("SELECT c_height FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_height'].'</td>';
		}
			$return .= '</tr>
						<tr>
							<th scope="row">Chest</th>';
	foreach($this->conn->query("SELECT c_chest FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_chest'].'</td>';
		}
			$return .= '</tr>
						<tr>
							<th scope="row">Waist</th>';
	foreach($this->conn->query("SELECT c_waist FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_waist'].'</td>';
		}
			$return .= '</tr>
						<tr>
							<th scope="row">Hips</th>';
	foreach($this->conn->query("SELECT c_hips FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_hips'].'</td>';
		}
			$return .= '</tr>
						<tr>
							<th scope="row">Thighs</th>';
	foreach($this->conn->query("SELECT c_thighs FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_thighs'].'</td>';
		}
			$return .= '</tr>
						<tr>
							<th scope="row">Biceps</th>';
	foreach($this->conn->query("SELECT c_biceps FROM client_measurements WHERE c_id = '".$c_id."' ORDER BY date_taken ASC LIMIT 5") as $row){
			$return .= '<td>'.$row['c_biceps'].'</td>';
		}
			$return .= '</tr>
					</tbody>
				</table>';
