...
class training{
	...
	public function display_routine_chart($id){
		//FOR CLIENT BACKEND
		...
		while ($row = $query->fetch(PDO::FETCH_ASSOC)){
			...
			$routine_table = '	<h2>'.$title.'</h2></br>
									<table>
										<tr><td style="width:30%;background-color:#448ccb;"><h3>Warm-up Exercises:</h3></td><td>'.$warm_up.'</td></tr>
			...
			routine_type_link.rt_id = routine_type.rt_id WHERE routine_type_link.ro_id = '".$id."'")as $row){
				...
				$routine_table .= '	<table></br></br>
										<tr><td><h2 style="color:#eeeeee;">'.$specific_type.' Exercises</h2></td></tr>
									
										<tr><th><h3>Exercises</h3></th><th><h3>Sets</h3></th><th><h3>Reps / Mins / Meters</h3></th></tr>
				...
				foreach($this->conn->query("SELECT * FROM routine_exercise INNER JOIN exercise ON routine_exercise.e_id = exercise.e_id
					...
					$routine_table .= '	
											<tr><td class="'.$burn.'"><p>'.$ex_title.'</p></td>
												
												<td><p style="text-align:center;">'.$ex_sets.'</p></td>
											<td><p style="text-align:center;">'.$ex_reps.' ('.$ex_reps_type.')</p></td></tr>';	
					}
				
				$routine_table .= '</table></br>';
			}
		$routine_table .= '<hr><hr><hr>';		
