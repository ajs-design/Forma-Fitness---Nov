	...
	public function current_goals($id){
		...
		foreach($this->conn->query("SELECT * FROM client_goals WHERE c_id = '".$id."'") as $row){
			$return .= '<div class="desc">
							'.$row['cg_goal'].'<em style="color:#cccccc;display:inline;">Target date: '.date("D jS F Y", strtotime($row['cg_target_date'])).'<br>
									Set date: '.date("D jS F Y", strtotime($row['cg_set_date'])).'
							</em>
							</div>
							<div class="controls">
